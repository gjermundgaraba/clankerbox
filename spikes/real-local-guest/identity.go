...
// Binding is delivered solely over the same-user administrative Unix socket
// through trusted engine exec. CA signing keys and host keys never enter guests.
type Binding struct {
...
func bindingConfig(b Binding) (*tls.Config, error) {
	if b.MachineID == "" || b.HostID == "" {
	...
	if _, err = leaf.Verify(x509.VerifyOptions{Roots: roots, Intermediates: intermediates, DNSName: "guest.clankerbox.internal"}); err != nil {
		return nil, err
	}
	if !hasURI(leaf, "spiffe://clankerbox/machine/"+b.MachineID) {
	...
	return &tls.Config{
		...
		VerifyConnection: func(s tls.ConnectionState) error {
			if len(s.PeerCertificates) == 0 || !hasURI(s.PeerCertificates[0], "spiffe://clankerbox/host/"+b.HostID) {
...
func (i *identity) rebind(b Binding) error {
...
func (i *identity) adminHandler() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := i.rebind(b); err != nil {
			http.Error(w, fmt.Sprintf("rebind: %v", err), 400)
			return
		}
...
func boundedServer(handler http.Handler) *http.Server {
	...
	p.SetHTTP1(true)
	p.SetHTTP2(true)
	return &http.Server{Handler: handler, Protocols: p, ReadHeaderTimeout: 5 * time.Second, IdleTimeout: 60 * time.Second, MaxHeaderBytes: 16 << 10}
}
