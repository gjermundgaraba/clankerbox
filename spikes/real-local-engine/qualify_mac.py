#!/usr/bin/env python3
import json,pathlib,subprocess,urllib.request,time,sys
C=pathlib.Path(__file__).resolve().parents[2]/'.work/real-local-engine/mac.json';cfg=json.loads(C.read_text());root=pathlib.Path(cfg['root']);driver=pathlib.Path(__file__).with_name('control.py')
def run(*a,ok=True):
...
(root/'mac-acceptance.json').write_text(json.dumps(report,indent=2))
...
(root/'mac-acceptance.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
