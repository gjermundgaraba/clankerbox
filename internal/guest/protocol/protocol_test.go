...
func TestValidation(t *testing.T) {
	...
	if err := bad.Validate(); !protocol.IsInvalid(err) {
		t.Fatalf("session id not validated: %v", err)
	}
	...
	input.Data = "***"
	if _, err = input.Validate(); !protocol.IsInvalid(err) {
		t.Fatalf("bad base64 accepted: %v", err)
	}
	report := protocol.ReportArgs{SessionID: good.SessionID, State: "asleep"}
	if err = report.Validate(); !protocol.IsInvalid(err) {
		t.Fatalf("unknown report state accepted: %v", err)
	}
}
