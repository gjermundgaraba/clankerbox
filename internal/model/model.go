...
func (p *Profile) Validate() error {
	...
	if !(p.Runtime == "tart" && p.OS == "macos" && p.Arch == "arm64") && !(p.Runtime == "smolvm" && p.OS == "linux") {
		return errors.New("unsupported OS/runtime combination")
	}
	if p.Runtime == "smolvm" && !SafePath(p.ImagePath) {
...
func (h Host) Validate() error {
	if !ValidName(h.ID) || !targetPattern.MatchString(h.SSHTarget) || !SafePath(h.HelperPath) || !SafePath(h.ConfigPath) || h.CPU < 1 || h.RAMMiB < 128 || len(h.ProfileIDs) == 0 {
