...
class RuntimeInputTests(unittest.TestCase):
    def setUp(self):
        ...
        for record in files:
            del record['mode']
        (inputs/'runtime-artifacts.json').write_text(json.dumps({'platforms': {
            'darwin-arm64': {'files': files},
            'linux-amd64': {'files': [dict(record, sha256='0'*64) if record['type'] == 'file' else record for record in files]},
        }}))
        self.args = SimpleNamespace(os='darwin', arch='arm64', engine=self.root/'engine',
                                    image=self.root/'image', runtime_assets=self.assets)
        ...
        (inputs/'pins.json').write_text(json.dumps({
            'hashes': {'target/debug/smolvm': bundle.sha(self.args.engine),
    ...
    def test_invalid_runtime_fails_before_output_or_build(self):
        ...
        argv = ['bundle.py', '--os', 'darwin', '--arch', 'arm64', '--version', 'test',
                '--engine', str(self.args.engine), '--image', str(self.args.image),
                '--runtime-assets', str(self.assets), '--engine-source', str(self.root),
                '--dependency-notices', str(self.root), '--output', str(output)]
    ...
    def test_changed_copy_fails_before_manifest(self):
        ...
        with mock.patch('sys.argv', argv), mock.patch.object(bundle, 'verify_licenses', return_value={}), \
                mock.patch.object(bundle.subprocess, 'run'), mock.patch.object(bundle.shutil, 'copytree', side_effect=changed_copy):
            with self.assertRaisesRegex(ValueError, 'qualified platform inventory'):
                bundle.main()
    ...
    def test_bundle_carries_project_license(self):
        ...
        notices.mkdir()
        output = self.root/'output'
        argv = ['bundle.py', '--os', 'darwin', '--arch', 'arm64', '--version', 'test', '--no-archive',
                '--engine', str(self.args.engine), '--image', str(self.args.image),
                '--runtime-assets', str(self.assets), '--engine-source', str(engine_source),
                '--dependency-notices', str(notices), '--output', str(output)]
        with mock.patch('sys.argv', argv), mock.patch('builtins.print'), \
                mock.patch.object(bundle, 'verify_licenses', return_value={}), \
                mock.patch.object(bundle, 'verify_release_licenses'), mock.patch.object(bundle.subprocess, 'run'):
            bundle.main()
