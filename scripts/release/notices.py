...
def collect(args):
    ...
    provenance = {
        ...
        'rust_lock_sha256': sha(engine / 'Cargo.lock'),
        'rust_metadata_sha256': sha(args.rust_metadata),
    }
...
def main():
    ...
    print(
        json.dumps(
            {'packages': len(rows), 'upstream_without_notice_files': [r['name'] for r in rows if not r['notices']]}
        )
    )
